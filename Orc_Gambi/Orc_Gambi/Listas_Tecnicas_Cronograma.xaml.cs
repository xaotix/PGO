...
namespace PGO
{
    ...
    public partial class Listas_Tecnicas_Cronograma : ModernWindow
    {
        public Listas_Tecnicas_Cronograma()
        {
            ...
            InitializeComponent();
            this.Lista_Ranges.ItemsSource = DLMorc.PGOVars.GetDbOrc().GetProdutos_Clean();
        }

        private void salvar_alteracoes(object sender, RoutedEventArgs e)
        {
            DLMorc.Produto sel = ((FrameworkElement)sender).DataContext as DLMorc.Produto;
            if (sel == null) { return; }
            sel.SalvarDatas();
        }
        ...
        {
            var sel = Lista_Ranges.SelectedItems.Cast<DLMorc.Produto>().ToList();
            if (sel.Count > 0)
            {
